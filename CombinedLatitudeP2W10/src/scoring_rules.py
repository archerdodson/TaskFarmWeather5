...
class SignatureEnergyCombined(ScoringRule):
    ...
    def signaturecompute(self, forecast: TensorType["batch", "ensemble_size", "data_size"],
        ...
        for latitude in range(latitudes):
            verificationlat = verification[:,:,latitude,:,:].squeeze(-1)/8
            forecastlat = forecast[:,:,latitude,:,:].squeeze(-1)/8
            ...
            score = signature_kernel.compute_scoring_rule(X,y)
            #print(score)
            totalscore.append(score*self.weights[latitude])
        #print(score)
        #requires double, not float?
